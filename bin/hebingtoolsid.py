    ...
    for i in range(length):
        if notexit(i,dele) == True:
            ...
            for j in range(length-1-i):
                if (abs(int(x2[i])-int(x2[j+1+i]))<5 and abs(int(x3[i])-int(x3[j+1+i]))<5) and x1[i] == x1[j+1+i] and x4[i] == x4[j+1+i]:
                    print('---------')
                    print('i=%d,j=%d',i,j)
                    print(x2[i])
                    print(x2[j+1+i])
...
y1,y2,y3,y4,y5 = chachong(x1,x2,x3,x4,x5)
y = []
#import os
...
for i in range(len(y1)):
    f1.write(str(y1[i])+"\t"+str(y2[i])+"\t"+str(y3[i])+"\t"+str(y4[i])+"\n")
f1.close()