...
===================================== 
<br>
<table><tr><font size = "4px">All results of softwares that users selected are showed in the Venn diagram below.In this plot, we can see the overlapping and the crossing of the results from different softwares clearly.
<br>
...
<br>
<table><tr><font size = "3px">After analysis, a table containing information of all diffientially expressed genes is presented with interactive options. Implication of nouns in header is explained in Table 1.
...
<br>
<table><tr><font size = "3px">The variance estimates plot is for checking the result of dispersion estimates adjustment.Specifically, in DESeq2, variance estimation is plotted by executing ```plotDispEsts()``` which is built-in in the package.
...
<br>
<table><tr><font size = "3px">Different samples may have different sequencing depth. In order to be comparable, it is necessary to estimate the relative size factors of each sample, and divide the samples by the size factors separately.    
...
<br>
<table><tr><font size = "3px">The different expression of the genes can be displayed intuitively in the *Heatmap*.The Heatmap can display data through color changes.The color of each grid represent the amout of the gene expression.The darker the color, the higher the expression.</td></tr></table></font>
<br>
```{r echo=FALSE,results='asis'}
